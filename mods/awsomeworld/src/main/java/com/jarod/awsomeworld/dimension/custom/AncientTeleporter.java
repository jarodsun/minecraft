    ...
    public Entity placeEntity(Entity entity, ServerWorld currentWorld, ServerWorld destiantionWorld,
        ...
        if (insideDimension){
            ...
            for (BlockPos checkPos : BlockPos.getAllInBoxMutable(destinationPos.down(10).west(10), destinationPos.up(10).east(10))){
                if (destiantionWorld.getBlockState(checkPos).getBlock() instanceof AncientTelBlock){
                    doSetBlock = false;
                    break;
                }
            }
            if (doSetBlock){
                destiantionWorld.setBlockState(destinationPos, ModBlocks.ANCIENT_TEL.get().getDefaultState());
            }
        }
        return entity;
    }

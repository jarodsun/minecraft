...
import com.jarod.awsomeworld.block.custom.RedstoneTelBlock;
...
public class RedstoneTeleporter implements ITeleporter {
    public static BlockPos thisPos = BlockPos.ZERO;

    public static boolean insideDimension = true;

    public RedstoneTeleporter(BlockPos pos, boolean insideDim){
        thisPos = pos;
    ...
    public Entity placeEntity(Entity entity, ServerWorld currentWorld, ServerWorld destiantionWorld,
        ...
        if (insideDimension){
            ...
            for (BlockPos checkPos : BlockPos.getAllInBoxMutable(destinationPos.down(10).west(10), destinationPos.up(10).east(10))){
                if (destiantionWorld.getBlockState(checkPos).getBlock() instanceof RedstoneTelBlock){
                    doSetBlock = false;
                    break;
                }
            }
            if (doSetBlock){
                destiantionWorld.setBlockState(destinationPos, ModBlocks.REDSTONE_TEL.get().getDefaultState());
            }
        }
        return entity;
    }
